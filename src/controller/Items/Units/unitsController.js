...
const createUnits = async (req, res, next) => {
    const { category, unit, quantity, usage_quantity } = req.body;
    ...
    //     // Check if the unit is valid for the 'quantity' category
    ...
    //     // Validate the quantity and usage_quantity
    //     const quantityAsNumber = Number(quantity);
    //     const usageQuantityAsNumber = Number(usage_quantity);
    ...
    //     if (!usage_quantity || isNaN(usageQuantityAsNumber) || usageQuantityAsNumber <= 0) {
    //         return responseSender(res, 422, false, "Invalid usage quantity. Usage quantity must be a positive number.");
    //     }
    ...
    try {
        ...
        const result = await pool.query(
            'INSERT INTO units (category, unit, quantity, usage_quantity) VALUES ($1, $2, $3, $4) RETURNING *',
            [category, unit, category === 'quantity' ? quantity : null, category === 'quantity' ? usage_quantity : null]
        );
...
const updateUnit = async (req, res, next) => {
    const { id, category, unit, quantity, usage_quantity } = req.body;
    ...
    try {
        ...
        // Update the unit
        const result = await pool.query(
            'UPDATE units SET category = $1, unit = $2, quantity = $3, usage_quantity = $4, updated_at = NOW() WHERE id = $5 RETURNING *',
            [category, unit, category === 'quantity' ? quantity : null, category === 'quantity' ? usage_quantity : null, id]
        );
